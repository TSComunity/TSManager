...
-# Para evitar este ping añadete el rol <@&1219196487011930194> en ⁠ <id:customize>.`)
        ...
        try {
            ...
            // ==========================================
            // ✅ SOLUCIÓN: Eliminar tarea anterior
            // ==========================================
            await tareasAsociaciones.deleteMany({ 
              channelId: message.channel.id 
            });
            console.log(`🗑️ Tareas anteriores eliminadas para canal ${message.channel.id}`);
            
            // Crear nueva tarea
            const expirationDate = new Date(Date.now() + doc.Renovacion * 86400000);
            await tareasAsociaciones.create({ 
              channelId: message.channel.id, 
              userId: asignado, 
              expirationDate: expirationDate 
            });
            console.log(`✅ Nueva tarea creada para canal ${message.channel.id}, expira: ${expirationDate}`);
